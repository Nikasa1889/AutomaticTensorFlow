...
fun fromInput tensor_nil = raise NA4
   ...
   |fromInput _ = raise NA1 (* input tensor must have operation placeholder and has no dependency *)
...
fun fullyConnect (tensor_nil, _) = raise NA4
   ...
   |fullyConnect (InputTensor as tensor_cons (_, _, _, dim_2(nRows, nColumns)), 
                  ScaleFactor) =  raise NA1
   |fullyConnect (InputTensor as tensor_cons (_, _, _, dim_3(_)), 
                  ScaleFactor) =  raise NA1

fun split (tensor_nil, _) = raise NA4
   |split (InputTensor as tensor_cons (_, _, _, dim_1(n)), SplitFactor) =
        if (SplitFactor <= 0.0 orelse SplitFactor >= 1.0) then raise NA3
        else 
            let
...
fun head (tensor_nil, _) = raise NA4
   |head (InputTensor as tensor_cons (_, _, _, dim_1(n)), nElems) =
        if (nElems <= 0.0 orelse floor(nElems) >= n) then raise NA3
        else 
            let
