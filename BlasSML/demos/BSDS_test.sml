...
in
 ...
 fun f( NInputs, NOutputs, LayerType ) = 
	consr(0.49714827779, consr (tanh(NOutputs), consr(0.152232676356, consr(~0.476040893294, consr(~1.07640110278, rnil)))))  
(*fun f( NInputs, NOutputs, LayerType ) =
  consr(
    0.49714827779, consr(
      case ~0.4889228302545773E9 of
        V81C7 => realMultiply( V81C7, V81C7 ),
      consr(
        ~0.476040893294,
      consr(  ~1.07640110278 , rnil )
        )
      )
    ) *)
...
val params:NN.params =  
    {batchsize = 100,   (* number of training cases per batch *)
     ...
     nItrs = 80,         (* number of iterations/epoches *)
...
val Inputs = [(25, 50, 50, 20, 2)]
val Validations = [(25, 50, 50, 20, 2)]
val input_files = [("../datasets/TinyBSDS/data1.csv", "../datasets/TinyBSDS/labels1.csv", 
        "../datasets/TinyBSDS/datavalid1.csv", "../datasets/TinyBSDS/labelsvalid1.csv")];
...
fun test (I)=
	output_eval_fun(I, main(25.0, 50.0, 50.0, 20.0, 2.0))
val _ = test(0)